package com.yuyue.agent.agents;
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        try {
            ...
            return Map.of(
                    ...
                    OUTPUT_IMAGE_REQUIREMENTS, new ArrayList<>()
            );
        }

        // 清理 LLM 响应（去除可能的 markdown 代码块标记和首尾空白）
        String cleanedJson = cleanJsonResponse(responseContent);
        log.debug("LLM 原始响应: {}", responseContent);
        log.debug("清理后 JSON: {}", cleanedJson);
